...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addItem(state, action) {
      ...
      if (!existingItem) {
        state.cartItems.push({
          id: newItem.idMeal,
          title: newItem.strMeal,
          image01: newItem.strMealThumb,
          price: newItem.price,
          quantity: 1,
    ...
    removeItem(state, action) {
      ...
      const existingItem = state.cartItems.find((item) => item.id === idMeal);
      state.totalQuantity--;

      if (existingItem.quantity === 1) {
        state.cartItems = state.cartItems.filter((item) => item.id !== idMeal);
      } else {
        existingItem.quantity--;
